...
Generell funktionieren die im folgenden vorgestellten Modelle mit jeder Art von (Bild-)Deskriptor: Die Modelle arbeiten genau aus diesem Grund nicht direkt mit den Bilddaten, sondern den Deskriptoren. Bei eben diesen handelt es sich lediglich um Vektoren aus dem Raum $\mathbb{R}^d$, wobei die Dimensionalität $d$ des Raums und damit die Anzahl der Elemente des Vektors abhängig vom Verfahren sind. Dies schränkt aber nicht die generelle Machbarkeit ein, sondern wirkt sich auf die Laufzeit bzw. den Speicherbedarf aus. Da für die vorliegenden Bilddaten der HsH keine speziellen Annahmen getroffen werden können, müsste mit einer Reihe verschiedener Deskriptoren experimentiert werden. Da eine Umsetzung aller geeigneten Deskriptoren den Rahmen dieser Arbeit sprengen würde, soll zunächst anhand einer Art von Deskriptor eine Basis geschaffen werden. Die zuvor vorgestellten Deskriptoren grenzen sich teilweise mehr, teilweise weniger voneinander ab: Der Spatial Envelope bietet einen ganz anderen Anwendungsfall als beispielsweise SIFT oder LBP. SIFT kann unter vielen Umständen Personen in Bildern erkennen, ist aber nicht darauf spezialisiert wie die LBP. Im Bereich der Objekterkennung ist SIFT jedoch der beste \glqq Allrounder\grqq.
...
\todo{Why unsupervised, dann Modelle}
...
Hinsichtlich der Verfahren zur Reduzierung von Dimensionen soll ein Autoencoder verwendet werden: Dieser bringt unter anderem dem Vorteil mit sich, das seine Architektur bereits auf eine parallele Verarbeitung ausgelegt ist und nicht \glqq extra\grqq berücksichtigt werden muss. Des weiteren sind klassische Methoden wie die Hauptkomponentenanaylse bereits zahlreich betrachtet worden. Ein Autoencoder hingegen ist in diesem Bereich ein wenig verbreiteter Vertreter.  

\todo{Mehr in allen Abschnitten}
