...
Es wurde bereits festgestellt, dass die Features in zweierlei Hinsicht ungeeignet durch die Verarbeitung klassischer Algorithmen sind:

\begin{itemize}
	\item Es liegt eine sehr große Menge von Features vor, die berücksichtigt werden soll. Maschinelle Lernverfahren eignen sich hier besonders, da sie zum einen auf eine große Menge an Trainingsdaten angewiesen sind, um ein nützliches Modell zu generieren und zum anderen eine parallele Verarbeitung begünstigen.
	\item Die Feature-Vektoren umfassen oft viele Komponenten, die zur Kodierung der Eigenschaften erforderlich sind. Im Bereich des Machine Learning gibt es Verfahren, welche die Feature-Vektoren auf ihre wesentlichen Komponenten analysieren und so eine kompaktere Darstellung erzeugen.
	\item \todo{Keine gelabelten Daten}
\end{itemize}

Insbesondere der dritte Punkt schließt überwachte Lernverfahren aus. Ein unüberwachter Ansatz erscheint auch intuitiv plausibler, da die Daten auf versteckte Strukturen hin untersucht werden. Tatsächlich fallen die ersten beiden Punkte je in einer Kategorie der unüberwachten Lernverfahren. Die  \textit{Clustering} Algorithmen befassen sich mit einer Gruppierung der Daten, um sie die Gesamtmenge zu reduzieren, der Bereich \textit{Dimensionality Reduction (Reduzierung der Dimensionalität} beschäftigt sich mit der Kompression von Daten. 
...
\begin{itemize}
	\item \textbf{Initiale Lernrate} Bei gradientenbasierten Vefahren wird in jeder Iteration der Fehler zurückpropagiert. Dies wird in den meisten Fällen zu einer zu schnellen Anpassung des Netzes führen und es \glqq vergisst\grqq   schnell, was es bereits gelernt hat. Aus diesem Grund werden die berechneten Fehler mit der Lernrate (\textit{learning rate}), einem kleinen Wert, multipliziert. Praktisch wird für die Lernrate ein Wert kleiner 1 und größer $10^{-6}$ verwendet.
	...
	\item \textbf{Anzahl der Exemplare pro Trainingsiteration} (\textit{batch size)} In der Praxis liegt der Wert für diesen Parameter zwischen 16 und 128 (Zweierpotenzen). Je größer die \textit{batch size}, desto mehr Trainingsexemplare werden pro Iteration verarbeitet. Durch einen größeren Wert kann die Berechnung beschleunigt werden, allerdings ist auch mehr Speicher erforderlich.
\end{itemize}
