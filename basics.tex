...
\subsection{Detektion und Extraktion von Features}
...
\begin{figure}[!htb]
	...
	\caption[Zwei Bilder mit dem gleichen Objekt aus unterschiedlicher Perspektive und in verschiedenen Lichtverhältnissen.]{Zwei Bilder mit dem gleichen Objekt aus unterschiedlicher Perspektive und in verschiedenen Lichtverhältnissen{\footnotemark}.}
	\label{img:liberty}
\end{figure}
...
\begin{figure}[!htb]
	...
	\caption{Schematische Darstellung der Subtraktion von Bildern im \textit{scale space} beim Difference of Gaussians Verfahren \cite{dif2004}.}
	\label{img:sift_dog}
\end{figure}	
	...
	\item \textbf{Bestimmung der Orientierung} Bei dem Aufbau der Feature-Vektoren pro \textit{keypoint} wird die lokale Orientierung berechnet. Auf diese Weise sind die SIFT Deskriptoren invariant gegenüber Rotationen. Der SIFT Algorithmus berechnet ein HOG. Hierfür werden zufällig Punkte aus der Nachbarschaft des \textit{keypoints} ausgewählt. Das Maximum des Histogramms wird dann als dominante Orientierung verwendet.
...
\begin{figure}[!htb]
	...
	\caption[Verarbeitung eines Signales in einem Neuron.]{Verarbeitung eines Signales in einem Neuron (ohne Ausgabefunktion).}
	\label{img:neuron}
\end{figure}
...
\begin{figure}[!htb]
	...
	\caption{Beispiel eines Autoencoders mit einem \textit{Hidden Layer}.}
...
\end{lstlisting}
\caption{Vektoraddition in CUDA C.}
\label{code:vektor} 
\end{figure}
