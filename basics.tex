...
SIFT ist ein Feature-Detektor und Deskriptor der 1999 von Lowe \cite{dif2004} entwickelt wurde. Neben der Invarianz der Skalierung der Features, berücksichtigt SIFT auch teilweise die Rotation, Beleuchtung und Perspektive. Der Algorithmus lässt sich in vier wesentliche Schritte unterteilt:
...
\begin{itemize}
	...
	\item \textbf{Aktivierungsfunktion} Es wird der neue Aktivierungszustand des Neurons aus dem alten Zustand und der Netzeingabe berechnet. Häufig wird hier eine SigmoidFunktion (\glqq S-Funktion\grqq) $s(x) = \frac{1}{1+e^{-x}}$ verwendet.
...
\begin{figure}
	...
	% Annotate edges
	\node[annot,above of=H-act, right=-0.3cm, node distance=0.2cm] {Netzeingabe};
	\node[annot,above of=H-sum, right=-0.3cm, node distance=0.2cm] {Aktivierung};
