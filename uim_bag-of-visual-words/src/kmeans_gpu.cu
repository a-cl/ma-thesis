...
void kmeans_gpu(float **points, float ** clusters, int *membership, const long count, const unsigned int size,
	...
	const unsigned int clusterBlockSharedDataSize = ThreadsPerBlock * sizeof(unsigned char) + k * size * sizeof(float);
	...
	cout << "  Shared Mem: " << clusterBlockSharedDataSize << " / " << prop.sharedMemPerBlock << endl;

	if (clusterBlockSharedDataSize > prop.sharedMemPerBlock) {
