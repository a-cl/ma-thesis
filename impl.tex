...
In diesem Abschnitt wird behandelt wie der Autoencoder in Python und TensorFlow umgesetzt wurde. Zunächst wird auf die Erhebung der Feature-Vektoren unter Verwendung der \textit{opencv}-Bibliothek eingegangen. Anschließend erfolgt eine Übersicht des Python-Codes zur Definition des Autoencoders.
...
Zunächst wird eine Übersicht über die Projektstruktur der Bag of Visual WordsKlasse Implementierung gegeben. Damit ein Modell über den Speicher hinaus verwendbar ist, wird ein einfacher Mechanismus zur Persistierung vorgestellt. Dem schließt die Betrachtung des k-means Clustering der Features an. Unterschiede sowie Limitierungen der \textit{global} und \textit{shared memory} Realisierung werden hier anhand des Codes aufgezeigt.
...
Zur Verwendung wird ein Objekt der BagOfVisualWords-Klasse durch Aufruf des Konstruktors angelegt. Hierbei muss die Anzahl der Cluster $k$ übergeben werden. Standardmäßig werden bei Erstellung und Nutzung eines Modells die CUDA Versionen des k-means und Histogramm Algorithmus aufgerufen. Zu Vergleichszwecken können aber auch die CPU-basierten Varianten durch \textit{setMode(1)} ($0 = GPU, 1 = CPU$) verwendet werden.\newline
Die Erstellung eines Modells erfolgt dann durch Aufruf von \textit{createModel(Mat features)} oder \textit{createModel(string featurePath)} erfolgen. Letztere liest aus der Datei \textit{featurePath} die zu gruppierenden Features ein. Pro Zeile wird ein Feature erwartet, dessen Komponenten durch Leerzeichen voneinander getrennt sind.\newline
Zur Erzeugung des Histogramms der \textit{Visual Words} dient die Methode \textit{computeViusalWords(Mat features)}. Auch hier gibt es eine überladene Funktion \textit{computeVisualWords(string featurePath)}, welche die Features aus der Datei \textit{featurePath} einließt und anschließend das Histogramm, bei entsprechendem Modus, durch die GPU berechnet. 

Damit ein Modell nicht immer erneut aufgebaut werden muss, kann es als Textdatei gespeichert werden. Dafür kann auf einem Objekt der BagOfVisualWords-Klasse \textit{writeModel(string modelPath)} aufgerufen werden. Die Schwerpunkte der Cluster werden zeilenweise in die Datei \textit{modelPath}/clusters geschrieben, sodass sich $k$ Zeilen mit 128 Elementen bei der Verwendung von SIFT ergeben. Die Mitgliedschaften der Features zu Cluster wird in der Datei \textit{modelPath}/membership gespeichert und enthält so viele Zeilen wie Features vorliegen. Jede Zeile enthält die $x$ und $y$ Koordinaten des \textit{keypoints} und den Index des zugehörigen Clusters. Der Index bezieht sich hierbei auf die Position des Clusters in der Datei \textit{modelPath}/clusters.
