...
\todo{main.py} In Zeile 8 wird die bereits eingeführte Funktion \textit{extractAll} genutzt, um alle Features der Bilder, die in \textit{imagePaths} enthalten sind, zu gewinnen.
...
trainPaths = sys.argv[1]
testPaths = sys.argv[2]

autoencoder.train(featureExtractor.extractAll(trainPaths))
autoencoder.encode(featureExtractor.extractAll(testPaths))
\end{lstlisting}
